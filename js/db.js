 ...
 */
function getItems(category, limit, offset, pager, view) {
	$.ajax({
		url : "db.php?function=getItems&category=" + category + "&limit=" + limit+ "&offset=" + offset,
		success : function(data) {
			...
			$("." + view).bind('click', function(e){
				...
				$('#popup').bPopup().css('top', $(this).css("top") - 600 + 'px');
			});
		},
		limit : limit
	});
...
function getCount(category) {
	var res;
	$.ajax({
		url : "db.php?function=getCount&category=" + category,
		success : function(data) {
			res = data;
		},
		async : false
